...
public class RedisRiver extends AbstractRiverComponent implements River {
  ...
  private class RedisPubSubRunner implements Runnable {
    ...
    public void run(){
      ...
      RiverListener listener = new RiverListener();

      this.jedis.subscribe(listener, redisKey);
    }

    class RiverListener extends JedisPubSub {

      private boolean processing = false;

      public RiverListener(){
        super();
      }
  ...
  private class RedisListRunner implements Runnable {
    ...
    private void loop() {
      ...
      try {
        this.jedis = jedisPool.getResource();  
        response = jedis.blpop(bulkTimeout, "lks:index");  
      } catch (Exception e) {
